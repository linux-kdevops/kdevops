...
def select_instance_from_tiers(
    ...
    Args:
        ...
        on_demand: If True, check on-demand/dynamic pricing availability instead of spot
        exclude: List of instance types to skip (e.g., instances that failed to deploy)
    ...
    for tier in tiers_to_check:
        ...
        for instance_type in instance_types:
            # Skip excluded instances (e.g., previously failed deployments)
            if instance_type in exclude:
                if verbose:
                    print(f"  Skipping {instance_type} (excluded)", file=sys.stderr)
                continue
...
""",
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="INSTANCE",
        help="Exclude instance type from selection (can be repeated). "
        "Use this to skip instances that failed to deploy.",
    )
    ...
    result = select_instance_from_tiers(
        args.tier_group, args.verbose, args.on_demand, args.exclude
    )
