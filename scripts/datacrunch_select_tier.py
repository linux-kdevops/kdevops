...
def get_all_available_capacity() -> Dict[str, List[str]]:
    """
...
def select_instance_from_tiers(
    ...
    # Get all available capacity across all regions once
    capacity_map = get_all_available_capacity()
    ...
    for tier in tiers_to_check:
        ...
        for instance_type in instance_types:
            ...
            if location:
                if verbose:
                    print(f"✓ AVAILABLE in {location}", file=sys.stderr)
                    print("", file=sys.stderr)
                    print(
                        f"Selected: {instance_type} in {location} (tier: {tier})",
                        file=sys.stderr,
                    )
