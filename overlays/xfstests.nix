...
in
{
  xfstests = prev.xfstests.overrideAttrs (prevAttrs: {
    ...
    #      fallback definition.
    #
    #   3. xfsprogs (when overridden to djwong's for-next or any
    #      newer tree carrying the same series) ships `struct
    #      file_attr` in <xfs/linux.h>. file_attr.c includes it
    #      transitively via "global.h" → <xfs/xfs.h>
    #      → <xfs/linux.h>.
    #
    # Combinations:
    #
    ...
    NIX_CFLAGS_COMPILE =
      (prevAttrs.NIX_CFLAGS_COMPILE or "") + " " + "-D__NR_file_getattr=468 -D__NR_file_setattr=469";
